		...
		*/
		...
		function checkHtmlSpanContent( $command) {
			...
			while( list( $rtf, $html) = each( $this->fontmodifier_table)) {
				if(isset($this->flags[$rtf]) && ($this->flags[$rtf] == true)) {
					if( $command == "start")
						$this->out .= "<".$html.">";
					else
						$this->out .= "</".$html.">";
		...
		*/
		function flushQueue() {
			if( strlen( $this->queue)) {
				// processing logic
				if(isset($this->flags["fonttbl_want_fcharset"]) && preg_match( "/^[0-9]+$/", $this->flags["fonttbl_want_fcharset"])) {
				...
				if( strlen( $this->queue)) {
					...
					*/
					...
					if( $this->wantHTML) {
						...
						if(isset($this->flags["fonttbl_current_read"]) && preg_match( "/^[0-9]+$/", $this->flags["fonttbl_current_read"])) {
